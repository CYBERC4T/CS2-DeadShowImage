        ...
        public override void Load(bool hotReload)
        {
            ...
            RegisterListener<Listeners.OnTick>(() =>
            {
                if (isShowImage)
                {
                    ...
                    {
                        if (player != null && !player.IsBot && !player.PawnIsAlive && !AdminManager.PlayerHasPermissions(player, settings.ImmunityFlag) && (player.Team == CsTeam.Terrorist || player.Team == CsTeam.CounterTerrorist || (settings.ShowImageForSpec && player.Team == CsTeam.Spectator)))
                        {
                            PrintHtml(player, $"{settings.HudTextFormat.Replace("{PATH}", currentImage)}");
                        }
                    }
                }
            });
